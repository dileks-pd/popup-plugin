	...
	class CpImport {
		...
		public function cp_import_preset_frontend( $module, $preset ) {
			...
			foreach ( $this->presets_list as $current_slug => $name ) {
				...
				$preset_atts = file_get_contents( $this->cp_preset_dir . $current_slug . '.txt' );
		 ...
		 */
		function cp_create_preset( $preset_atts, $current_slug, $slug_id, $attr ) {
		 ...
		 */
		function cp_upload_image( $dir, $source, $file_name ) {
			...
			if ( ! is_wp_error( $temp_file ) ) {
				...
				$file_content = file_get_contents( $results['file'] );
