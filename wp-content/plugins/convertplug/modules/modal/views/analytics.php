			...
			<div class="col-sm-2">
				...
				<select id="style-dropdown" multiple>
					...
					?>
						...
						<option value="<?php echo $value['style_id']; ?>" 
								<?php
								if ( in_array( $value['style_id'], $style_array ) ) {
									echo "selected = 'selected'";}
								?>
								><?php echo $style_name; ?></option>
								<?php } ?>
								<?php
								if ( isset( $value['style_id'] ) ) {
									if ( is_array( $smile_variant_tests ) && isset( $smile_variant_tests[ $value['style_id'] ] ) ) {
										foreach ( $smile_variant_tests[ $value['style_id'] ] as $key => $variant_test ) {
											...
											<option data-variant='true' value="<?php echo  $style_id; ?>" 
												<?php
												if ( in_array( $style_id, $style_array ) ) {
													echo "selected = 'selected'";}
												?>
													><?php echo urldecode( stripslashes( $style_name ) ); ?></option>";
													<?php
										}
									}
								}
												...
												<div class="col-sm-2">
													...
													<select id="cp-chart-comp-type">
														...
														if ( 'imp' === $comp_factor ) {
															echo "selected='selected'";}
														?>
															><?php _e( 'Impression', 'smile' ); ?> </option>
															<option value="conv" 
															<?php
															if ( 'conv' === $comp_factor ) {
																echo "selected='selected'";}
															?>
																><?php _e( 'Conversion', 'smile' ); ?></option>
